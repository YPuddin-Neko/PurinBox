...
use super::{collect_image_files, ProgressEvent};
...
pub async fn scan_sd_metadata(
    ...
    tokio::task::spawn_blocking(move || scan_sync(&app, &input_path))
        .await
...
fn scan_sync(app: &tauri::AppHandle, input_path: &str) -> Result<SdScanResult, String> {
    ...
    for (i, file_path) in files.iter().enumerate() {
        ...
        let _ = app.emit("sd-metadata-progress", ProgressEvent {
            current: i as u32 + 1,
            total,
...
fn export_sync(app: &tauri::AppHandle, options: &ExportTagsOptions) -> Result<ExportResult, String> {
...
fn read_png_metadata(path: &PathBuf) -> Option<RawMeta> {
    ...
    while pos + 12 <= buf.len() {
        let length = u32::from_be_bytes([buf[pos], buf[pos + 1], buf[pos + 2], buf[pos + 3]]) as usize;
    ...
    for (key, value) in &text_chunks {
        if key == "Comment" && value.trim_start().starts_with('{') {
            let source_ver = text_chunks.iter()
                .find(|(k, _)| k == "Source")
                .map(|(_, v)| v.as_str())
                .unwrap_or("");
    ...
    for (_node_id, node) in obj {
        ...
        if class_type == "KSampler" || class_type == "KSamplerAdvanced" {
            ...
            if let Some(pos_link) = inputs.get("positive").and_then(|v| v.as_array()) {
                if let Some(pos_node_id) = pos_link.first().and_then(|v| v.as_str()).or_else(|| pos_link.first().and_then(|v| v.as_u64()).map(|_| "")) {
            ...
            if let Some(neg_link) = inputs.get("negative").and_then(|v| v.as_array()) {
                if let Some(neg_id) = neg_link.first() {
                    ...
                    if let Some(neg_node) = obj.get(&neg_id_str) {
                        if let Some(text) = neg_node.get("inputs").and_then(|i| i.get("text")).and_then(|t| t.as_str()) {
...
fn parse_novelai(comment: &str, source_ver: &str) -> Option<RawMeta> {
    ...
    if !source_ver.is_empty() {
        ...
        let clean_ver = source_ver.rsplit_once(' ')
    ...
    if let Some(chars) = obj.get("v4_prompt")
        .and_then(|v| v.get("caption"))
    ...
    if let Some(refs) = obj.get("director_reference_descriptions").and_then(|v| v.as_array()) {
        for r in refs {
            if let Some(cap) = r.get("caption").and_then(|c| c.get("base_caption")).and_then(|v| v.as_str()) {
                if !cap.is_empty() {
