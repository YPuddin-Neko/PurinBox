...
pub async fn blur_noise_images<R: tauri::Runtime>(
    ...
    let _busy = crate::commands::BusyGuard::acquire(&RUNNING, "模糊噪点")?;

    CANCEL_FLAG.store(false, Ordering::SeqCst);
    tokio::task::spawn_blocking(move || blur_noise_sync(&app, &options))
        .await
...
fn blur_noise_sync<R: tauri::Runtime>(
    ...
    for (i, file_path) in files.iter().enumerate() {
        ...
        match process_blur_noise(file_path, input, output_dir, options) {
            Ok(_) => {
                success_count += 1;
                let _ = app.emit(
                    "blur-noise-progress",
...
fn process_blur_noise(
    ...
    if options.noise_strength > 0 {
        ...
        // 使用文件名hash做伪随机种子
        let mut seed: u64 = file_path
            .to_string_lossy()
            .bytes()
            .fold(0u64, |acc, b| acc.wrapping_mul(31).wrapping_add(b as u64));

        let strength = options.noise_strength as f64;
