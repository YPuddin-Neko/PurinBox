...
| 功能 | 说明 |
| --- | --- |
| **图片打标** | Tagger 模型（WD / CL）自动生成 Danbooru 风格标签，LLM 生成自然语言描述，支持 CPU/GPU 推理、递归扫描、TXT/JSON 输出 |
| **标签管理** | 可视化编辑标签，支持批量添加/删除/替换、拖拽排序、公共标签筛选、全局标签翻译、标签去重、Danbooru 标签自动补全、预览大图和双击编辑，兼容 TXT/JSON 格式 |
