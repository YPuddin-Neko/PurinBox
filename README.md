...
| 功能 | 说明 |
| ------ | ------ |
| **图片打标 (Tagger)** | 使用 WD Tagger 模型自动生成 Booru 风格标签，支持 CPU/GPU 推理，自动检测系统 Python 环境 |
| **图片打标 (LLM)** | 使用大语言模型（OpenAI 兼容 API）生成自然语言描述，支持耗时统计与失败图片自动归档 |
| **标签排序** | 使用 LLM 对标签文件进行语义排序，支持并发处理、即时取消、标签对比验证，错误/警告文件自动归档 |
| **标签管理** | 可视化编辑标签，支持批量添加/删除/替换、拖拽排序、公共标签筛选、标签翻译、实时刷新、多维度排序 |
| **分桶预览** | 预览训练集图片的 Bucket 分桶结果，支持 3×3 图片预览、桶内滚动浏览、导出分桶结构 |

### ⚙️ 系统功能

- 🌙 深色/浅色/跟随系统 三种主题模式
- 🌐 标签翻译（Google、百度、有道、Bing 四种翻译引擎），支持全局目标语言设置
- 🔌 网络代理设置（HTTP/SOCKS5，支持 LLM 独立代理开关）
- 📊 实时系统监控（CPU、内存）
- 💾 翻译结果本地缓存（SQLite）
- 🔑 API 配置持久化，预设 OpenAI / Gemini / DeepSeek 端点，密钥安全显隐
- 🐍 Python 环境自动管理，优先使用系统已安装版本

### 🔮 未来高级工具预告

| 功能 | 说明 |
| ------ | ------ |
| **图片聚类** | 基于视觉特征对数据集图片进行自动聚类分组，发现重复或相似图片 |
| **图片人像拆分** | 自动检测多人图片并拆分为单人图片，适用于角色训练数据集制作 |
